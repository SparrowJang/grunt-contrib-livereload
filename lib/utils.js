...
utils.livereloadSnippet = function livereloadSnippet(req, res, next) {
  ...
  res.end = function (string, encoding) {
    ...
    unzip( tmpBuffer, encoding, function( string, encoding ){
      ...
      //if content type is html,it will inject livereload script.
      if( res._headers !== undefined && /html/.test( res._headers["content-type"] ) ) inject(string, encoding);

      else res.push( string && string.toString ? string.toString() : "" );

      restoreWriteHead();

      if (res.data !== undefined ) {

        //if( !res._headers ) res.setHeader('content-length', Buffer.byteLength( new Buffer( res.data ) ,encoding ));
